...
def plot_mosaic(imgs, plot_fn, aspect_ratio=1.3333, shape=None,
    ...
    # Massage params
    def ensure_list(val, max_iters, val_name=None):
        if (not isinstance(val, collections.Iterable)
            ...
            raise ValueError("Lengths don't match (%d != %d)%s" % (
                len(val), max_iters, ' (%s)' % val_name if val_name else ''))
    plot_fn = ensure_list(plot_fn, n_plots, 'plot_fn')
    for arg_name, arg_val in kwargs.items():
        kwargs[arg_name] = ensure_list(arg_val, n_plots, arg_name)
    ...
    for fi, (img, cur_fn, plot_args) in enumerate(zip(imgs, plot_fn, kwargs_list)):
        ax = figure.add_subplot(shape[0], shape[1], fi + 1)
