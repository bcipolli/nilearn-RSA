...
conditions_encoded, sessions = np.loadtxt(haxby_dataset.session_target).astype("int").T
conditions = np.recfromtxt(haxby_dataset.conditions_target)['f0']
condition_mask = np.ones(conditions.shape, dtype='bool')
...
n_samples = (conditions == 'house').sum()
n_conditions = len(np.unique(conditions))
grouped_fmri_masked = np.empty((n_sessions * n_conditions * n_samples,  # two conditions
                                fmri_masked.shape[1]))
grouped_conditions_encoded = np.empty((n_sessions * n_conditions * n_samples, 1))

idx = 0
for si in range(n_sessions):
    session_mask = sessions[condition_mask] == si
    for ci, cond in enumerate(np.unique(conditions)):
        cond_mask = np.logical_and(session_mask,
                                   conditions[condition_mask] == cond)
        n_events = len(conditions_encoded[cond_mask])
        grouped_fmri_masked[idx:(idx + n_events)] = fmri_masked[cond_mask]
        grouped_conditions_encoded[idx:(idx + n_events)] = conditions_encoded[cond_mask][0]
        idx += n_events
...
print("Performing massively univariate analysis...")
...
threshold = -np.log10(0.01)  # 10% corrected
